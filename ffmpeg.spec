...
%{!?_without_frei0r:BuildRequires: pkgconfig(frei0r)}
%{?_with_gme:BuildRequires: game-music-emu-devel}
BuildRequires:  pkgconfig(gnutls)
BuildRequires:  gsm-devel
%{?_with_ilbc:BuildRequires: pkgconfig(libilbc)}
BuildRequires:  lame-devel >= 3.98.3
#"pkgconfig(jack)" = pipewire-jack-audio-connection-kit-devel
%{!?_without_jack:BuildRequires: jack-audio-connection-kit-devel}
%{!?_without_ladspa:BuildRequires: ladspa-devel}
%{!?_without_aom:BuildRequires: pkgconfig(aom)}
%{!?_without_dav1d:BuildRequires: pkgconfig(dav1d) >= 0.2.1}
%{!?_without_ass:BuildRequires: pkgconfig(libass)}
%{!?_without_bluray:BuildRequires: pkgconfig(libbluray)}
%{?_with_bs2b:BuildRequires: pkgconfig(libbs2b)}
%{?_with_caca:BuildRequires: pkgconfig(caca)}
%{!?_without_cdio:BuildRequires: pkgconfig(libcdio_paranoia)}
%{?_with_chromaprint:BuildRequires: pkgconfig(libchromaprint)}
%{?_with_crystalhd:BuildRequires: libcrystalhd-devel}
%{?_with_lensfun:BuildRequires: lensfun-devel}
%if 0%{?_with_ieee1394}
...
%{?_with_smb:BuildRequires: pkgconfig(smbclient)}
BuildRequires:  pkgconfig(libssh)
BuildRequires:  pkgconfig(theora)
BuildRequires:  pkgconfig(libv4l2)
%{?!_without_vaapi:BuildRequires: pkgconfig(libva) >= 0.31.0}
BuildRequires:  pkgconfig(vdpau)
BuildRequires:  pkgconfig(vorbis)
%{?_with_vapoursynth:BuildRequires: pkgconfig(vapoursynth)}
%{?!_without_vpx:BuildRequires: pkgconfig(vpx) >= 1.4.0}
%{?_with_mfx:BuildRequires: pkgconfig(libmfx) >= 1.23-1}
%ifarch %{ix86} x86_64
BuildRequires:  nasm
%endif
%{?_with_webp:BuildRequires: pkgconfig(libwebp)}
%{?_with_netcdf:BuildRequires: pkgconfig(netcdf)}
%{?_with_rpi:BuildRequires: raspberrypi-vc-devel}
%{!?_without_nvenc:BuildRequires: pkgconfig(ffnvcodec)}
%if %{with amr}
...
%{!?_without_lv2:BuildRequires:  pkgconfig(lilv-0) pkgconfig(lv2)}
%{!?_without_openal:BuildRequires: pkgconfig(openal)}
%if 0%{!?_without_opencl:1}
BuildRequires:  opencl-headers pkgconfig(ocl-icd)
%{?fedora:Recommends: opencl-icd}
%endif
%{?_with_opencv:BuildRequires: pkgconfig(opencv)}
BuildRequires:  pkgconfig(libopenjp2)
%{!?_without_opus:BuildRequires: pkgconfig(opus) >= 1.1.3}
%{!?_without_pulse:BuildRequires: pkgconfig(libpulse)}
BuildRequires:  perl(Pod::Man)
%{?_with_rav1e:BuildRequires: pkgconfig(rav1e)}
%{?_with_rubberband:BuildRequires: pkgconfig(rubberband)}
%{!?_without_tools:BuildRequires: pkgconfig(sdl2)}
%{?_with_snappy:BuildRequires: pkgconfig(snappy)}
BuildRequires:  pkgconfig(soxr)
BuildRequires:  pkgconfig(speex)
BuildRequires:  pkgconfig(srt)
%{?_with_svtav1:BuildRequires: svt-av1-devel >= 0.9.0}
%{?_with_tesseract:BuildRequires: pkgconfig(tesseract)}
#BuildRequires:  texi2html
BuildRequires:  texinfo
%{?_with_twolame:BuildRequires: pkgconfig(twolame)}
%{?_with_vmaf:BuildRequires: pkgconfig(libvmaf) >= 1.5.2}
%{?_with_wavpack:BuildRequires: pkgconfig(wavpack)}
%{!?_without_vidstab:BuildRequires:  pkgconfig(vidstab)}
%if %{with vulkan}
BuildRequires:  vulkan-loader-devel
BuildRequires:  pkgconfig(shaderc) >= 2019.1
%endif
%{!?_without_x264:BuildRequires: pkgconfig(x264) >= 0.0.0-0.31}
%{!?_without_x265:BuildRequires: pkgconfig(x265)}
%{!?_without_xvid:BuildRequires: xvidcore-devel}
%{!?_without_zimg:BuildRequires: pkgconfig(zimg) >= 2.7.0}
BuildRequires:  zlib-devel
%{?_with_zmq:BuildRequires: pkgconfig(libzmq)}
